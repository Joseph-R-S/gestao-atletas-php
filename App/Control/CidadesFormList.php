...
/**
    ...
    {
        // 1. Executa o carregamento padrão da Trait.
        // Aqui ela abre a transação, busca as cidades, fecha a transação e joga na Grid.
        $this->onReloadTrait();   

        // 2. Agora a transação da Trait fechou, mas a Grid vai falhar ao rodar o show() depois.
        // Para resolver, abrimos uma transação rápida AQUI para carregar os Estados na memória.
        try {
            Transaction::open($this->connection);
            
            // Pegamos todos os itens que a Trait acabou de colocar na Datagrid
            $items = $this->datagrid->getItems();
            if ($items) {
                foreach ($items as $cidade) {
                    // Força a chamada do método mágico que carrega o Estado 
                    // enquanto esta nossa transação está ativa!
                    $cidade->nome_estado; 
                }
            }
