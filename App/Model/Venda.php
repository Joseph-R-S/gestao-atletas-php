...
class Venda extends Record
{
     ...
     */
    public function get_cliente() : Pessoa
    {
     ...
     */
    public function addItem(Produto $p, int|float $quantidade)
    {    
        $item = new ItemVenda;
        $item->produto    = $p;
     ...
     */
    public function get_itens() : array
    {
     ...
     */
    public static function getVendasMesSqlite()
    {
        $meses = array();
        $meses[1] = 'Janeiro';
        $meses[2] = 'Fevereiro';
    ...
    public static function getVendasMes()
    {
        ...
        // Cambiado strftime() por EXTRACT(MONTH FROM ...) compatible con PostgreSQL
        $result = $conn->query("SELECT EXTRACT(MONTH FROM data_venda) as mes, sum(valor_final) as valor 
                                FROM venda 
        ...
        foreach ($result as $row)
        {
            // EXTRACT retorna el número de mes, lo convertimos a entero para usarlo de índice
            $mesIndex = (int) $row['mes'];
            
            if (isset($meses[$mesIndex])) {
                $mes = $meses[$mesIndex];
                $dataset[ $mes ] = (float) $row['valor'];
            }
        }
